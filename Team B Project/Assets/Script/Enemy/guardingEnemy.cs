...
public class guardingEnemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        else if ((enemy == null || !visionChecker.isInVision || Vector2.Distance(transform.position, pointA.transform.position) > 10f) && currentState != EnemyState.Guarding)
        {
            Debug.Log("Returning to guard position");
            currentState = EnemyState.Guarding; // Transition to guarding
        }
    ...
    private void Guard()
    {
        if (Vector2.Distance(transform.position, pointA.transform.position) > 1f) // anggap 1f sebagai toleransi jarak jaga
        {
            MoveTowards(pointA.transform.position);
            anim.SetBool("IsWaiting", false);  // Pastikan animasi walking atau running aktif jika sedang bergerak
        }
        else
        {
            m_Rigidbody2D.velocity = Vector2.zero;
            anim.SetBool("IsWaiting", true);  // Set animasi ke idle
        }
    }
    ...
    private void MoveTowards(Vector2 target)
    {
        ...
        m_Rigidbody2D.velocity = direction * walkSpeed;
